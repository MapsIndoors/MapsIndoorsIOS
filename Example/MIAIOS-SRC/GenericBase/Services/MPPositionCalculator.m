...
- (MPPoint*) calcLatLngPos:(NSArray*) measurements {
    ...
    MPPointXY* pos = [self calcMyPos:measurements];
    MPPointXY* zeroPos = [[MPPointXY alloc] init];
    ...
    CLLocationCoordinate2D destCoord = GMSGeometryOffset( [MPPointXY getRefPoint], distance, bearing );
...
- (double) calcErrorMargin:(MPPointXY*)pos measurements:(NSArray*)measurements {
    double result = 0;
    for( MPPointXY* p in measurements ) {
        result += fabs(p.distance - [self distanceBetween: p and:pos]);
    }
    return result;
}

- (double) distanceBetween:(MPPointXY*)p1 and:(MPPointXY*)p2 {
    double dx = p2.X - p1.X;
    double dy = p2.Y - p1.Y;
...
-(MPPointXY*) calcMyPos:(NSArray*) measurements {
    MPPointXY *guess = [self calcFirstGuess:measurements];
...
-(MPPointXY*) calcFirstGuess:(NSArray*) measurements {
    MPPointXY* result = [[MPPointXY alloc] init];
    for ( MPPointXY* b in measurements)
    {
        result.X += b.X;
        result.Y += b.Y;
    }
...
- (void) calcNewGuess:(MPPointXY*) position measurements:(NSArray*)measurements
{
    double jtj[2][2];
    jtj[0][0] = 0;
    jtj[0][1] = 0;
    ...
    MPPointXY* correction = [self multiply:jtj JTf:jtf];
    position.X -= correction.X;
    position.Y -= correction.Y;
}

//Calculate inverse (Jacobian transposed * Jacobian)
-(void)calcJTJ:(NSArray*) measurements guessPos:(MPPointXY*) guessPos result:(double[2][2]) result
{
    double jtj[2][2];
    jtj[0][0] = 0;
    jtj[0][1] = 0;
    ...
    jtj[1][1] = 0;
    //Calc JTJ
    for ( MPPointXY* m in measurements)
    {
...
- (void)calcJTf:(NSArray *) measurements  guessPos:(MPPointXY*) guessPos result:(double[2][2]) result
{
    //Calc JTJ
    for (MPPointXY* m in measurements)
    {
...
- (MPPointXY*)multiply:(double[2][2]) JTJ JTf:(double[2][2]) JTf {
    MPPointXY* result = [MPPointXY alloc];
